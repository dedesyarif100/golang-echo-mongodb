...
type Repository struct {
	DB *mongo.Collection
	DBTransaction *mongo.Collection
	DBMenu	*mongo.Collection
}
...
func NewMerchantRepository(DB *mongo.Database) MerchantRepository {
	return &Repository{
		...
		DBTransaction: DB.Collection("transaction"),
		DBMenu: DB.Collection("menu"),
	}
}
...
func (repo *Repository) GetAllMerchant() ([]entity.MerchantResult, float64, error) {
	...
	// CRUD OPERATIONS
		// READ OPERATIONS
			...
			// ---------------------------------------------------------------------------------------------
				...
				// opts := options.Find().SetSort(bson.D{{Key: "rating", Value: -1}}).SetSkip(1).SetLimit(2)
				// opts := options.Find().SetLimit(2).SetSort(bson.D{{Key: "rating", Value: -1}}).SetSkip(1)
				// opts := options.Find().SetLimit(2).SetSkip(1).SetSort(bson.D{{Key: "rating", Value: -1}})
				// opts := options.Find().SetSkip(1).SetSort(bson.D{{Key: "rating", Value: -1}}).SetLimit(2)
			...
			// ---------------------------------------------------------------------------------------------
				...
				// 	{Key: "email", Value: true},
				// 	{Key: "pic_email", Value: true},
				// 	{Key: "logo_url", Value: true},
				// 	{Key: "address", Value: true},
				// })
				// filter := bson.D{
				// 	{Key: "name", Value: "admin"},
				// 	{Key: "address", Value: "jkt"},
				// 	{Key: "logo_url", Value: "en"},
				// }
				// cursor, err := repo.DB.Find(ctx, filter, opts)
				// var results []entity.MerchantResult
				// cursor.All(ctx, &results)
				...
				// 	{Key: "$limit", Value: 5},
				// }
				// cursor, err := repo.DB.Aggregate(ctx, mongo.Pipeline{filter, projectStage})
				// var results []entity.MerchantResult
				// cursor.All(ctx, &results)
				// if err != nil {
				// 	return nil, float64(len(results)), err
	...
	// -------------------------------------------------------------------------------------------------------------------
		...
		// _, err := repo.DBMenu.InsertMany(context.TODO(), docs)
		// ------------------------------------------------------------------------
		...
		// 			{Key: "$avg", Value: "$price"},
		// 		}},
		// 		{Key: "type_total", Value: bson.D{
		// 			{Key: "$sum", Value: 1},
		// 		}},
		// }}}
		// cursor, _ := repo.DBMenu.Aggregate(ctx, mongo.Pipeline{groupStage})
		// var tr []bson.M
		// cursor.All(ctx, &tr)
